...
제한된 시스템 자원을 효율화하고 중단 없는 24시간 운영을 위해 아래 11가지 항목을 설정하고 적용했습니다.
...
*   **분석 (Analysis):**
    * 시스템 요인: 안드로이드 딥 도즈(Deep Doze) 진입 시, 시스템이 '앱 최적화 예외' 설정을 무시하고 통신 모뎀(LTE/5G) 전원을 강제 차단함에 따라 물리적 송출 경로가 소멸됨.

    * 한계점 포착: 배터리 15% 미만의 임계 상황에서는 커널의 하드웨어 보호 정책이 최상위 우선순위를 가지며, 이로 인해 `PersistentKeepalive` 패킷조차 시스템에 의해 드롭(Drop)되는 현상 확인.
...
*   **결과 (Result):**
    * 기술적 한계 확인: 소프트웨어적 설정만으로는 딥 도즈 상황에서의 `network is unreachable` 에러 및 물리적 단절을 완전히 극복할 수 없음을 실증함.

    * 가용성 전략 수립: 극단적 저전력 상황에서 VPN 핸드쉐이크 지연이 카카오톡 등 필수 알림 수신까지 방해하는 병목 현상을 확인하였으며, 향후 배터리 임계치에 따른 VPN 자동 해제 등 '통신 가용성 우선 정책'으로의 전환 근거를 마련함.
---
